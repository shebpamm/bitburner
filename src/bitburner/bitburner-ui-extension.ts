...
import { initBatching } from "./batch";
...
export async function main(netscript: NS) {
  ...
  const spans = tailWindows
    .map((win) => [...win.querySelectorAll("span")])
    .flat();
  ...
  initBatching(netscript);

  await new Promise<void>((resolve) =>
    watchElForDeletion(windowEl, () => resolve()),
  );

  sendEvent({ uuid: "test", data: { action: "die" } });
    ...
    mutations.forEach(function (mutation) {
      ...
      if (mutation.type === "childList") {
        mutation.removedNodes.forEach(
          (node) => !containsRecursive(node, elToWatch) || callback(),
        );
      }
    });
  });
